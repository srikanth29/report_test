...
$lastweek_from = date("Y-m-d H:i:s", strtotime("-1 day", strtotime($from)));
#echo $lastweek_from;
$lastweek_to = date("Y-m-d H:i:s", strtotime("-1 day", strtotime($to)));
#echo $lastweek_to;
/*date_default_timezone_set("Europe/London");
	$to = date("Y-m-d H:i:s");
	$from = date("Y-m-d H:i:s", time() - 60 * 5);*/
...
$sql_api_quick = "SELECT COUNT(*) as freq FROM `log` WHERE API='DO_QUICK_TOPUP_RESPONSE' && Error_code='0' && Time_stamp BETWEEN '$from' AND '$to' GROUP BY API" ;
...
$quick_count = $row['freq'] ;
 ...
 $sql_api_quicklast = "SELECT COUNT(*) as freq FROM `log` WHERE API='DO_QUICK_TOPUP_RESPONSE' && Error_code='0' && Time_stamp BETWEEN '$lastweek_from' AND '$lastweek_to' GROUP BY API" ;
...
$sql_api_purchase = "SELECT COUNT(*) as freq FROM `log` WHERE API='PURCHASE_FREE_SIM_RESPONSE' && Error_code='0' && Time_stamp BETWEEN '$from' AND '$to' GROUP BY API" ;
...
$purchase_count = $row['freq'] ;
 ...
 $sql_api_purchaselast = "SELECT COUNT(*) as freq FROM `log` WHERE API='PURCHASE_FREE_SIM_RESPONSE' && Error_code='0' && Time_stamp BETWEEN '$lastweek_from' AND '$lastweek_to' GROUP BY API" ;
...
$sql_api_get = "SELECT COUNT(*) as freq FROM `log` WHERE API='GET_ACCOUNT_DETAILS_RESPONSE ' && Error_code='0' && Time_stamp BETWEEN '$from' AND '$to' GROUP BY API" ;
...
$get_count = $row['freq'] ;
 ...
 $sql_api_getlast = "SELECT COUNT(*) as freq FROM `log` WHERE API='GET_ACCOUNT_DETAILS_RESPONSE ' && Error_code='0' && Time_stamp BETWEEN '$lastweek_from' AND '$lastweek_to' GROUP BY API" ;
...
<div class="container">
 <div class="row ">
   <div class="col-sm-12">
    <div class="well well-lg"><h3 style="text-align: center;">Lycamobile API statistics</h3></div>
    </div>
  </div>
</div>
<div class="container">
...
<h2><div class="well well-sm">Comparison over last week</div></h2>
 <div class="row">
   ...
   <div class="col-sm-4">
     <h3 class="display-3"><span class="label label-primary">Get account detials</span></h3>
